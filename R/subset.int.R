...
###	internal function for data subsets
	...
	start <- strptime(subset[1], "%Y-%m-%d %H:%M:%S", tz)
	end <- strptime(subset[2], "%Y-%m-%d %H:%M:%S", tz)
	...
	match.date <- difftime(timestamp, ISOdatetime(1,1,1,0,0,0), tz=tz, units="days") - difftime(start, ISOdatetime(1,1,1,0,0,0), tz=tz, units="days")
	start <- which(abs(as.numeric(match.date)) == min(abs(as.numeric(match.date))))	
	match.date <- difftime(timestamp, ISOdatetime(1,1,1,0,0,0), tz=tz, units="days") - difftime(end, ISOdatetime(1,1,1,0,0,0), tz=tz, units="days")
